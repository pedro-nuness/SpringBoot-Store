package com.hype.application.service.user.userCart;
...
import com.hype.application.dto.user.userCart.CartItemDTO;
...
import com.hype.application.repository.user.userCart.userCartItemRepository;
import com.hype.application.repository.user.userCart.userCartRepository;
    ...
    public CartResponseDTO getCartByUserId(String userId) {
        ...
        List<CartItemDTO> items = cart.getItems().stream().map(item ->
                new CartItemDTO(
                        ...
                        item.getProduct().getImages().stream().map(ImageResponseDTO::new).toList(),
                        item.getQuantity()
                )).toList();
